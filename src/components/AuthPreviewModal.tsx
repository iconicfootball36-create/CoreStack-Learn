...
import { X, ShieldCheck, Mail, Lock, User, Sparkles, ArrowRight, CheckCircle2, AlertCircle, BookOpen, Compass, Zap } from 'lucide-react';
...
export const AuthPreviewModal: React.FC<AuthModalProps> = ({ isOpen, mode: initialMode, onClose, onSuccess }) => {
  ...
  const handleDemoLogin = async () => {
    ...
    try {
      ...
      setError(err.message || 'Failed to login demo user.');
    } finally {
      ...
      <div className="relative w-full max-w-lg max-h-[92vh] overflow-y-auto p-6 sm:p-8 rounded-2xl bg-white border border-slate-200 text-slate-900 shadow-2xl space-y-6">
        ...
        <div>
          <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-blue-50 border border-blue-100 text-blue-700 text-[11px] font-mono font-semibold uppercase mb-3">
            ...
            <span>Phase 2: Live Student Authentication</span>
          </div>
        ...
        {/* 1-Click Demo & Google Sign-In Actions */}
        <div className="space-y-2.5">
          ...
          {/* 1-Click Demo Sign-In Action */}
          <div className="p-3 rounded-xl bg-blue-50/60 border border-blue-200 flex flex-col sm:flex-row items-center justify-between gap-3">
            <div className="text-left">
              ...
              <p className="text-[11px] text-blue-700">
                Sign in as <strong>Alex Rivera</strong> (Pre-loaded OS & DB notes)
              </p>
            </div>
            <button
              type="button"
              ...
              className="w-full sm:w-auto whitespace-nowrap px-3.5 py-1.5 rounded-lg bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold transition-all shadow-sm cursor-pointer disabled:opacity-50"
            >
              {isDemoLoading ? 'Signing In...' : '1-Click Demo'}
            </button>
          </div>
        </div>
